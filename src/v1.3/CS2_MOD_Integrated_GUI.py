...
UI_TEXT = {
    "ja": {
        ...
        "output_csv_dir": "出力CSVフォルダ:",
        "browse": "参照...",
        "do_copy": "JSONをコピーする",
        "do_excel": "Excel / スプレッドシートへ変換する",
        "do_csv": "CSVに変換する",
        "run": "実行",
        ...
        "pick_csv_dir_title": "出力CSVフォルダを選択",
        "error_title": "エラー",
        "done_title": "完了",
        ...
        "err_no_csv_dir": "出力CSVフォルダが未指定です。",
        "start_log": "[INFO] 開始",
        "done_msg": "処理が完了しました。",
    ...
    "en": {
        ...
        "output_csv_dir": "Output CSV Folder:",
        "browse": "Browse...",
        "do_copy": "Copy JSON",
        "do_excel": "Export to Excel / Spreadsheet",
        "do_csv": "Export to CSV",
        "run": "Run",
        ...
        "pick_csv_dir_title": "Select Output CSV Folder",
        "error_title": "Error",
        ...
        "err_no_csv_dir": "Output CSV folder is not specified.",
...
def build_normalized_frames(json_path: Path, log_fn=print) -> Dict[str, pd.DataFrame]:
    """PdxModsCache.json を正規化して、各DataFrameを返す。"""
    ...
    for key, rec in root.items():
        ...
        # Mods シート用
        mod_row: Dict[str, Any] = {"Key": key}
        for field_name in scalar_fields:
            mod_row[field_name] = safe_text(rec.get(field_name))
        mods_rows.append(mod_row)

        # Tags シート用
        tags_obj = rec.get("Tags")
        if isinstance(tags_obj, dict):
            for tag_key, tag_value in tags_obj.items():
                tag = tag_key if tag_key is not None else tag_value
                if tag is not None:
        ...
        if isinstance(req_list, list):
            for req in req_list:
                ...
                req_key_union.update(req.keys())
                req_row: Dict[str, Any] = {"Key": key}
                for k2, v2 in req.items():
                    req_row[k2] = safe_text(v2)
        ...
        if isinstance(dlc_list, list):
            for dlc in dlc_list:
                ...
                dlc_key_union.update(dlc.keys())
                dlc_row: Dict[str, Any] = {"Key": key}
                for k2, v2 in dlc.items():
                    dlc_row[k2] = safe_text(v2)
                dlc_rows.append(dlc_row)
        ...
        if isinstance(imgs, list):
            for image_item in imgs:
                ...
                img_key_union.update(image_item.keys())
                img_row: Dict[str, Any] = {"Key": key}
                for k2, v2 in image_item.items():
                    img_row[k2] = safe_text(v2)
        ...
        if isinstance(links, list):
            for link_item in links:
                ...
                link_key_union.update(link_item.keys())
                link_row: Dict[str, Any] = {"Key": key}
                for k2, v2 in link_item.items():
                    link_row[k2] = safe_text(v2)
                link_rows.append(link_row)

    frames = {
        "Mods": pd.DataFrame(mods_rows),
        "Tags": pd.DataFrame(tags_rows) if tags_rows else pd.DataFrame(columns=["Key", "Tag"]),
        "Requirements": build_df(req_rows, req_key_union),
        "DlcRequirements": build_df(dlc_rows, dlc_key_union),
        "Images": build_df(img_rows, img_key_union),
        "Links": build_df(link_rows, link_key_union),
    }
...
def export_json_to_excel(json_path: Path, out_xlsx: Path, log_fn=print) -> None:
    ...
    frames = build_normalized_frames(json_path, log_fn=log_fn)
...
def export_json_to_csv(json_path: Path, out_csv_dir: Path, log_fn=print) -> None:
    """PdxModsCache.json を正規化して複数CSV出力する。"""
    if not json_path.exists():
        raise FileNotFoundError(f"入力JSONが見つかりません:\n{json_path}")

    if not out_csv_dir.exists():
        log_fn(f"[INFO] CSV出力フォルダを作成: {out_csv_dir}")
        out_csv_dir.mkdir(parents=True, exist_ok=True)

    frames = build_normalized_frames(json_path, log_fn=log_fn)

    for sheet_name, df in frames.items():
        out_csv = out_csv_dir / f"{sheet_name}.csv"
        log_fn(f"[INFO] CSV出力: {out_csv}")
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.do_csv_var = tk.BooleanVar(value=False)
        ...
        default_csv_dir = default_copy_dir / "csv_export"
        ...
        self.csv_dir_var.trace_add("write", self.on_setting_changed)
    ...
    def _build_ui(self):
        """画面部品を作成する。"""
        # 上部タイトル + Version表示 + 言語切替
        top = tk.Frame(self)
        top.pack(fill="x", padx=10, pady=(10, 4))

        self.version_label = tk.Label(top, text="")
        self.version_label.pack(side="left", anchor="w")
        ...
        self.label_json = tk.Label(frm1, text="", width=20, anchor="w")
        self.label_json.pack(side="left")

        self.entry_json = tk.Entry(frm1, textvariable=self.json_var)
        self.entry_json.pack(side="left", fill="x", expand=True, padx=6)
        ...
        self.label_copy_dir = tk.Label(frm2, text="", width=20, anchor="w")
        self.label_copy_dir.pack(side="left")

        self.entry_copy_dir = tk.Entry(frm2, textvariable=self.copy_dir_var)
        self.entry_copy_dir.pack(side="left", fill="x", expand=True, padx=6)
        ...
        self.label_xlsx = tk.Label(frm3, text="", width=20, anchor="w")
        self.label_xlsx.pack(side="left")

        self.entry_xlsx = tk.Entry(frm3, textvariable=self.xlsx_var)
        self.entry_xlsx.pack(side="left", fill="x", expand=True, padx=6)

        self.btn_xlsx = tk.Button(frm3, text="", command=self.pick_xlsx)
        self.btn_xlsx.pack(side="left")

        # 出力CSVフォルダ
        frm_csv = tk.Frame(self)
        frm_csv.pack(fill="x", padx=10, pady=8)

        self.label_csv_dir = tk.Label(frm_csv, text="", width=20, anchor="w")
        self.label_csv_dir.pack(side="left")

        self.entry_csv_dir = tk.Entry(frm_csv, textvariable=self.csv_dir_var)
        self.entry_csv_dir.pack(side="left", fill="x", expand=True, padx=6)

        self.btn_csv_dir = tk.Button(frm_csv, text="", command=self.pick_csv_dir)
        self.btn_csv_dir.pack(side="left")
        ...
        self.chk_csv = tk.Checkbutton(
            frm4,
            text="",
            variable=self.do_csv_var,
            command=self.save_settings,
        )
        self.chk_csv.pack(side="left", padx=8)
        ...
        self.log = tk.Text(frm6, height=20)
        self.log.pack(fill="both", expand=True)

    def _apply_language(self):
        """表示言語をUIへ反映する。"""
        self.title(f"{self.tr('window_title')}  Version {APP_VERSION}")
        self.version_label.config(text=self.tr("version_label", version=APP_VERSION))

        self.language_label.config(text=self.tr("language_label"))
        self.language_combo.configure(
            values=[self.tr("language_ja"), self.tr("language_en")]
        )
        self.language_var.set(
            self.tr("language_en") if self.lang == "en" else self.tr("language_ja")
        )
        ...
        self.label_csv_dir.config(text=self.tr("output_csv_dir"))
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.csv_dir_var.set(data.get("csv_dir", self.csv_dir_var.get()))
            ...
            self.do_csv_var.set(bool(data.get("do_csv", self.do_csv_var.get())))
        except Exception:
            # 設定ファイルが壊れていても起動は継続
            pass
    ...
    def pick_csv_dir(self):
        """出力CSVフォルダを選ぶ。"""
        p = filedialog.askdirectory(title=self.tr("pick_csv_dir_title"))
        if p:
            self.csv_dir_var.set(p)
            self.save_settings()
    ...
    def open_out_dir(self):
        ...
        if self.xlsx_var.get().strip():
            candidates.append(Path(self.xlsx_var.get()).parent)

        if self.csv_dir_var.get().strip():
            candidates.append(Path(self.csv_dir_var.get()))

        if self.copy_dir_var.get().strip():
            candidates.append(Path(self.copy_dir_var.get()))
    ...
    def run_clicked(self):
        ...
        copy_dir = Path(self.copy_dir_var.get()) if self.copy_dir_var.get().strip() else Path()
        out_xlsx = Path(self.xlsx_var.get()) if self.xlsx_var.get().strip() else Path()
        out_csv_dir = Path(self.csv_dir_var.get()) if self.csv_dir_var.get().strip() else Path()

        if (
            not self.do_copy_var.get()
            and not self.do_excel_var.get()
            and not self.do_csv_var.get()
        ):
        ...
        if self.do_csv_var.get() and not self.csv_dir_var.get().strip():
            messagebox.showerror(self.tr("error_title"), self.tr("err_no_csv_dir"))
            return
